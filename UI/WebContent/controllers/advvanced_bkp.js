pharmApp.controller('AdvancedSettingsCtrl', ['$scope','$rootScope','$http','$modal','$log','$timeout','ngToast', function ($scope,$rootScope,$http, $modal, $log,$timeout,ngToast) {
	...
	$scope.init=function(){
		...
		//call for userLIst
		var json={"entity":2,"entityId":0,"listType":2,"requestData":""/*,uid:userSelectedId*/,"userId":$scope.activeUser.userId,"token":$scope.activeUser.token};
		//HTTP CALL FOR 
		$http({
			method: 'POST',
	...
	$scope.getUserData=function(userSelectedId){
		var json={"entity":2,"entityId":0,"listType":1,"requestData":"","uid":userSelectedId/*userSelectedId*/,"userId":$scope.activeUser.userId,"token":$scope.activeUser.token};
		//HTTP CALL FOR 
		$http({
			method: 'POST',
			url: "http://localhost:8080/Purple/rest/apiAdvanceSetting/getAdvanceSetting",
	...
	$scope.filterGrids=function(type){
		if(type=='role'){
			...
			if($scope.permissionFilterStatus=='ON'){
				...
				w2ui.grid5.search([{ field:'purpleaidACLIsPermission', value:'true', operator:'is',type:'text'}]);
			}
		}
	};
	$scope.save=function(){
		var json={"entity":1,"entityId":0,"listType":0,"requestData":"","uid":userSelectedId/*userSelectedId*/,"userId":$scope.activeUser.userId,"token":$scope.activeUser.token};
		//HTTP CALL FOR 
		$http({
			method: 'POST',
