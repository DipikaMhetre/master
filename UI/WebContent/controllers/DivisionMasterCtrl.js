pharmApp.controller('DivisionMasterCtrl', ['$scope','$rootScope','$http','$modal','$log', function ($scope,$rootScope,$http, $modal, $log) {
	...
	$scope.init=function(){
		...
		var config = {
			    ...
			    grid: { 
			        ...
			        columns: [
			  	            { field: 'divisionName', caption: 'Division Name', size: '100%',style: 'text-align: left',resizable: false,sortable: true},
			  	            { field: 'divisionCompanyName', caption: 'divisionCompanyName', hidden:true },
			  	            { field: 'divisionCompanyId', caption: 'divisionCompanyId', hidden:true },
			  	            { field: 'divisionCode', caption: 'divisionCode', hidden:true },
			  	            ...
			  	            { field: 'divisionActiveFlag', caption: 'divisionActiveFlag', hidden:true },
			        ...
			        onSelect: function(event) {
			        	...
			        	angular.element(document.getElementById('divisionMaster')).scope().selectDivision(w2ui.grid.get(event.recid));
		...
		$http({
		    method: 'POST',
		    url: "http://localhost:8080/Purple/rest/apiDivision/getDivision",
		    async:false,
			...
			if(data.responseCode == 200){
				...
				$scope.masterDivisionList=respData.divisionList;
	...
	$scope.onCompanySelection=function(companySelected){
		if(companySelected.companyId==0){
			...
			w2ui.grid.records=$scope.masterDivisionList;
			w2ui.grid.refresh();
	...
	$scope.selectDivision=function(divisionSelected){
		debugger;
		$scope.showDivisionSelected=divisionSelected;
		$scope.$apply();
	}
	
	$scope.saveDivisionMaster=function(selectedDivision){
		var a=DivisionValidate();
		if(a==false)
		 {
		  return false;
		 }
		if(typeof selectedDivision === "undefined"){ 
			return false;
		};
		
		var flag='update';
		debugger;
		if(typeof selectedDivision.divisionId === "undefined"){
			if(!$scope.showCompanySelected){
				alert("Please Select Company In which New Division to Add")
				return false;
			}
			...
			selectedDivision.divisionCompanyId=$scope.showCompanySelected.companyId;
			selectedDivision.divisionCompanyName=$scope.showCompanySelected.companyName;
		}
		debugger;
		var json=selectedDivision;
		...
		$http({
			method: 'POST',
			url: "http://localhost:8080/Purple/rest/apiDivision/setDivision",
			async:false,
			...
			if(data.responseCode == 200){
					...
					angular.forEach($scope.showCompanyList, function(value,key) {
						if(keepGoing) {
							if(value.companyId == returnObj.divisionCompanyId){
								index = key;
								keepGoing = false;
							}
						}
					...
					/*====================for new====================================*/
					if(flag=='new'){
						$scope.showCompanyList[index].companyDivisionsList.push(returnObj);
						//$scope.showDivisionList.push(returnObj);
						$scope.onCompanySelection($scope.showCompanyList[index]);
						$scope.masterDivisionList.push(returnObj);
						w2ui.grid.select(returnObj.recid);
					}else{
						/*====================for Edited====================================*/
						indexSec=null;
						keepGoing = true;
						angular.forEach($scope.showCompanyList[index].companyDivisionsList, function(value,key) {
							if(keepGoing) {
								if(value.divisionId == returnObj.divisionId){
									indexSec = key;
									keepGoing = false;
								}
							}
						});
						returnObj.$$hashKey=$scope.showCompanyList[index].companyDivisionsList[indexSec].$$hashKey
						$scope.showCompanyList[index].companyDivisionsList[indexSec]=returnObj;
									
						indexSec=null;
						keepGoing = true;
						angular.forEach($scope.masterDivisionList, function(value,key) {
							if(keepGoing) {
								if(value.divisionId == returnObj.divisionId){
									indexSec = key;
									keepGoing = false;
						...
						$scope.onCompanySelection($scope.showCompanyList[index]);
						returnObj.$$hashKey=$scope.masterDivisionList[indexSec].$$hashKey
						$scope.masterDivisionList[indexSec]=returnObj;
					}
			}
			else{
	...
	$scope.newDivisionMaster=function(){
		...
		$scope.showDivisionSelected={};
		$scope.showDivisionSelected.divisionActiveFlag=true;
		$scope.showDivisionSelected.divisionCompanyName=$scope.showCompanySelected.companyName;
	};
...
function DivisionValidate(){
	flag=true;
	if($.trim($('#divisionname').val()).length==0){
		$("#divisionname").css("background-color", "#fff4eb");
		//create error message using tooltip
		flag=false;
	}
	if($.trim($('#divisioncode').val()).length==0){
		$("#divisioncode").css("background-color", "#fff4eb");
		//create error message using tooltip
		flag=false;
	}
	if(angular.element(document.getElementById('divisionMaster')).scope().errorCount==0){
