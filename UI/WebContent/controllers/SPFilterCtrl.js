...
pharmApp.controller('SPFilterCtrl', ['$scope','$rootScope','$http','$modal','$log','$timeout','ngToast', function ($scope,$rootScope,$http, $modal, $log,$timeout,ngToast) {
	$scope.labelList=[];
	...
	$scope.init=function(){
		...
		var parentScope=angular.element(document.getElementById('SystemPermission')).scope();
	...
	$scope.ok=function(filter){
		angular.copy(filter,angular.element(document.getElementById('SystemPermission')).scope().mainFilter);
		if(!angular.equals(filter,$scope.clearFilter)){
			angular.element(document.getElementById('SystemPermission')).scope().mainFilterStatus='ON';
		}else{
			angular.element(document.getElementById('SystemPermission')).scope().mainFilterStatus='OFF';
		}
		angular.element(document.getElementById('SystemPermission')).scope().sortAndFilter();
		$scope.$close();
	};
